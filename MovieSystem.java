...
class PasswordEncoder {
    ...
    public static String encodePassword(String rawPassword) {
        try {
            ...
            ExceptionHandler.handleException("Password encryption failed", e);
            return rawPassword;
        }
    }
    ...
    public static boolean verifyPassword(String rawPassword, String storedPassword) {
        try {
            ...
            ExceptionHandler.handleException("Password verification failed", e);
            return rawPassword.equals(storedPassword);
        }
    }
...
class ExceptionHandler {
    ...
    public static void handleCsvFormatException(String line, int lineNumber) {
        System.err.println("[ERROR] The" + lineNumber + "Row CSV format invalid ：" + line);
    }
}
...
public class MovieSystem {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(movieFilePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                try {
                    ...
                    ExceptionHandler.handleException("Error parsing movie line " + lineCount + ": " + line, e);
                }
            }
            System.out.println("Successfully loaded " + movies.size() + " movies from " + movieFilePath);

        } catch (FileNotFoundException e) {
            ExceptionHandler.handleException("Movie file not found: " + movieFilePath, e);
        } catch (IOException e) {
            ExceptionHandler.handleException("Error reading movie file", e);
        }
    }
    ...
    private Movie parseMovieFromCsv(String csvLine) {
        ...
        if (parts.length < 5) {
            ExceptionHandler.handleRuntimeException("Invalid movie data (insufficient columns): " + csvLine);
            return null;
        }
        ...
        try {
            ...
            ExceptionHandler.handleException("Error parsing numbers in movie data: " + csvLine, e);
            return null;
        } catch (Exception e) {
            ExceptionHandler.handleException("Unexpected error parsing movie data: " + csvLine, e);
            return null;
        }
    }
